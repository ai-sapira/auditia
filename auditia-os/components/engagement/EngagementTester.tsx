...
export const EngagementTester: React.FC<EngagementTesterProps> = ({ onBack }) => {
  ...
  const getStatusDisplay = (status: InvoiceStatus, item: SampleInvoice) => {
    switch (status) {
      case 'pending':
        return <span className="text-[10px] text-stone-400">Pendiente</span>;
      case 'reviewed':
        return (
      ...
      case 'sap_fetching':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">
      ...
      case 'sap_fetched':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-purple-700 bg-purple-50 px-2 py-0.5 rounded border border-purple-100">
      ...
      case 'waiting_proof':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-orange-700 bg-orange-50 px-2 py-0.5 rounded border border-orange-100">
    ...
    <div className="space-y-6 animate-fade-in h-full flex flex-col">
       ...
       <div className="flex items-center gap-4 pb-6 border-b border-stone-200 shrink-0">
          ...
          <div className="flex items-center gap-3">
             ...
             <div>
                <h2 className="text-2xl font-serif text-stone-900">Muestreo Alternativo</h2>
                <span className="text-xs text-stone-400 font-mono">Tester Agent</span>
             </div>
          </div>
       </div>
       ...
       <div className="grid grid-cols-12 gap-6 flex-1 overflow-hidden min-h-0">
          ...
          <div className="col-span-8 flex flex-col gap-6 overflow-y-auto pr-2">
              ...
              <div className="grid grid-cols-4 gap-px bg-stone-200 border border-stone-200 rounded-sm overflow-hidden">
                  ...
                  <div className="bg-white p-4">
                      <span className="text-[10px] text-purple-600 uppercase tracking-wider font-sans block mb-1">Con SAP</span>
                      <div className="text-2xl font-serif text-purple-600 tabular-nums">{sapFetchedCount}</div>
                      <span className="text-[10px] text-stone-500">docs extraídos</span>
                  </div>
                  <div className="bg-white p-4">
                      <span className="text-[10px] text-orange-600 uppercase tracking-wider font-sans block mb-1">Esperando</span>
                      <div className="text-2xl font-serif text-orange-600 tabular-nums">{waitingProofCount}</div>
                      <span className="text-[10px] text-stone-500">comprobantes</span>
                  </div>
              </div>
                          ...
                          {sample[selectedInvoice] && (
                              <>
                                  ...
                                  {sample[selectedInvoice].sapDocument && (
                                    <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
                                      <div className="flex items-center gap-2 mb-3">
                                        ...
                                        <span className="text-xs font-medium text-purple-800">Documento SAP</span>
                                      </div>
                                      <div className="space-y-2 text-xs">
                                        <div className="flex justify-between">
                                          <span className="text-purple-600">Nº Documento:</span>
                                          <span className="font-mono text-purple-900">{sample[selectedInvoice].sapDocument.invoiceNumber}</span>
                                        </div>
                                        <div className="flex justify-between">
                                          <span className="text-purple-600">Condiciones pago:</span>
                                          <span className="text-purple-900">{sample[selectedInvoice].sapDocument.paymentTerms}</span>
                                        </div>
                                        <div className="flex justify-between">
                                          <span className="text-purple-600">Importe:</span>
                                          <span className="font-mono text-purple-900">{sample[selectedInvoice].sapDocument.amount.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €</span>
                                        </div>
                                      </div>
                                    </div>
                                  )}
                                  ...
                                  {sample[selectedInvoice].clientResponse && (
                                    <div className={`mb-6 p-4 rounded-lg border ${sample[selectedInvoice].clientResponse!.isPaid ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}>
                                      <div className="flex items-center gap-2 mb-3">
                                      ...
                                      <div className="space-y-2 text-xs">
                                        ...
                                        <div className="flex justify-between">
                                          ...
                                          <span className={sample[selectedInvoice].clientResponse!.proofUploaded ? 'text-emerald-600 font-medium' : 'text-orange-600'}>{sample[selectedInvoice].clientResponse!.proofUploaded ? 'Sí ✓' : 'Pendiente'}</span>
                                        </div>
                                      </div>
                                    </div>
                                  )}
                                  ...
                                  <div className="space-y-2">
                                      <span className="text-xs font-medium text-stone-700 block mb-2">Acciones:</span>
                                      ...
                                      {!sample[selectedInvoice].sapDocument && (
                                        <button
                                            ...
                                            className="w-full px-4 py-2 bg-purple-50 border border-purple-200 text-purple-700 text-xs font-medium rounded-sm hover:bg-purple-100 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                                        >
                                      ...
                                      {sample[selectedInvoice].sapDocument && !sample[selectedInvoice].clientResponse && (
                                        <button
                                            ...
                                            className="w-full px-4 py-2 bg-blue-50 border border-blue-200 text-blue-700 text-xs font-medium rounded-sm hover:bg-blue-100 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                                        >
                                            {sendingToClient ? (
                                              <>
                                                <Loader2 className="w-4 h-4 animate-spin" />
                                      ...
                                      <div className="border-t border-stone-200 pt-3 mt-3">
                                        <span className="text-[10px] text-stone-500 block mb-2">Marcar como:</span>
                                        <div className="space-y-2">
                                          <button
             ...
             >
               {sapFetching ? (
                 <div className="text-center py-8">
                   ...
                   />
                   <h3 className="text-lg font-serif text-stone-900 mb-2">Conectando con SAP...</h3>
                   <p className="text-sm text-stone-500">Extrayendo documento de factura del ERP</p>
                 ...
                 <div className="text-center py-8">
                   ...
                   <h3 className="text-lg font-serif text-stone-900 mb-2">Documento extraído</h3>
                   <p className="text-sm text-stone-500">La factura ha sido recuperada de SAP correctamente.</p>
                   <button
                     onClick={() => setShowSAPModal(false)}
                     className="mt-6 px-6 py-2 bg-stone-900 text-white text-sm font-medium rounded-lg hover:bg-black transition-colors"
                   >
                     Continuar
                   </button>
                 </div>
         ...
         {showClientModal && selectedInvoice !== null && (
           ...
           >
             ...
             >
               <div className="flex justify-between items-start mb-6">
                 <div>
                   <h3 className="text-lg font-serif text-stone-900">Simular respuesta del cliente</h3>
                   <p className="text-sm text-stone-500 mt-1">¿La factura ha sido pagada?</p>
                 </div>
                 <button 
               ...
               <div className="flex gap-3">
                 <button
                   ...
                   className="flex-1 px-4 py-3 bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-medium rounded-lg hover:bg-emerald-100 transition-colors flex flex-col items-center gap-1"
                 >
                   <CreditCard className="w-5 h-5" />
                   <span>Sí, pagada</span>
                   <span className="text-[10px] text-emerald-500">Se solicitará comprobante</span>
                 </button>
                 <button
                   onClick={() => handleClientResponse(selectedInvoice, false)}
                   className="flex-1 px-4 py-3 bg-amber-50 border border-amber-200 text-amber-700 text-sm font-medium rounded-lg hover:bg-amber-100 transition-colors flex flex-col items-center gap-1"
                 >
                   <Truck className="w-5 h-5" />
                   <span>No, pendiente</span>
                   <span className="text-[10px] text-amber-500">Se solicitará albarán</span>
                 </button>
               </div>
             </motion.div>
           </motion.div>
